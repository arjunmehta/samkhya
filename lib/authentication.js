...
// function newSession(sessionID, userID, callBack){

//   var sessionID = generateUniqueSessionID(userID);
...
//       if(typeof callBack === "function") callBack (err, sessionID, userID);
//     });
//   }
//   else{
//     if(typeof callBack === "function") callBack (err, sessionID, userID);
...
function addNewSession(sessionID, userID, callBack){
...
function getRequestSessionInfo(req, callBack){
    ...
    if(config.options.redisStore){
      ...
      if(typeof callBack === "function") callBack(req.session.sessionID, req.session.userID);
    }  
}

function getConnectionSessionInfo(whichOne, callBack){
...
function userSessionExists(reqType, source, callBack){
  ...
  authentication.getSessionInfo(reqType, source, function (userName, sessionID, userID){
    ...
    if(authentication.userSessions[userID]){
      if(typeof callBack === "function") callBack("local", userName, userID, userID);
    }
    else{
      if(config.options.redisStore){
        samsaara.client.hexists("userSessions", userID, function(err, reply){
          if(reply == 1){
            if(typeof callBack === "function") callBack("foreign", userName, sessionID, userID);
          }
          else{
