...
function initialize(samsaaraCore, modules){
  ...
  (function middleware(module){
    ...
    function finalizeModule(middlewareExport){     
      if(middlewareExport.finalizationMethod){
        middlewareExport.finalizationMethod();
      }
    }
    ...
    function initializeModule(middleware){
      ...
      if(e.routeMessageOverride && typeof e.routeMessageOverride === "function"){
        routeMessageOverride(moduleName, e.routeMessageOverride);
      }
    ...
    function routeMessageOverride(moduleName, method){
      samsaaraCore.router.routeMessage = method;
    }
    ...
    function initializeBaseObjects(moduleName, objects){
      ...
      for(var object in objects){
        if(!samsaara[object]){
          ...
          throw new Error("Foundation method: " + object + " is already an internal method on samsaara");
        }
      }
    }
