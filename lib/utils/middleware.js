...
var middlewareLoader = {
    ...
    use: function(module, options) {
        ...
        if (typeof module.name === 'string' && typeof module.initialize === 'function') {
            ...
            modules[module.name].options = options;
        } else {
            console.error('Invalid samsaara middleware module loaded.');
        }
...
function addRemoteMethods(moduleName, remoteMethods) {
...
function initializeModule(middleware, options) {
    ...
    var initializedModule = middleware.initialize(generateClosuredExtender, capability, options);
...
function generateClosuredExtender(moduleName) {
    ...
    return {
        ...
        addRemoteMethods: function(remoteMethods) {
            addRemoteMethods(moduleName, remoteMethods);
        },
