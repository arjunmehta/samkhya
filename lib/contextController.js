...
samsaara.contextFunc(contextID, "something", propertyA, )


this.contexts[contextID].send("something", propertyA, propertyB, function(err, reply){
  
});

Context.prototype.func = function(fName){
  ...
  isContextOpen(contextID, function (open, local){
    ...
    if(open === false){
      ...
      if(redisStore === true){
        ...
        config.redisClient.hset("openContexts", contextID, process.pid);
        config.redisSub.subscribe("CTX:" + contextID);
        config.emit("openedContext", contexts[contextID]);       
      }
      else{
        createContext(contextID, contextData, options.access);
        config.emit("openedContext", contexts[contextID]);
...
var openContext = ContextController.prototype.openContext = function(contextID, callBack){
    ...
    isContextOpen(contextID, function (open, local){
      ...
      if(open === false){
        ...
        if(typeof callBack === "function") callBack(null, contextID, done);
        ...
        if(typeof callBack === "function") callBack("ERR: " + contextID + "already open", contextID, done);
      }
...
//             }
//             else{
//               if(typeof callBack === "function") callBack("Not Open", false);
//             }
//           });
//         }
//         else{
...
var switchContext = ContextController.prototype.switchContext = function (connID, contextID, callBack){
  ...
  clearFromContext(connID, function (whichID){
    ...
    if(context === undefined && connectionOwner !== process.pid){
      ...
      communication.sendToOwner(whichID, connectionOwner, {ns: "samsaara", func: "switchContext", args: [whichID, contextID], specialKey: config.specialKey}, callBack);
    }
    else{
      ...
      isContextOpen(contextID, function (open, local){
        ...
        if(open === true){
          ...
          if(typeof callBack === "function") callBack("Context Not Open", false);
        }
...
var updateContext = ContextController.prototype.updateContext = function(contextID, callBack){
  ...
  config.redisClient.hset("contextList", contextID, projectData, function(err, reply){
    var contextRow = JSON.parse(reply);

    if(typeof callBack === "function") callBack(contextID, reply);
    config.emit("contextUpdated", whichOne, contextID);
  });
};
...
var addToForeignContext = ContextController.prototype.addToForeignContext = function(whichOne, whichContext, callBack){
  ...
  var foreignMessage = { ns: "samsaara", func: "switchContext", args: [connID, whichContext], specialKey: config.specialKey };
  ...
  communication.sendToContext(symbolic, whichContext, foreignMessage, callBack);
...
var clearFromContext = ContextController.prototype.clearFromContext = function (connID, callBack){
  ...
  if(contextID !== null && contexts[contextID] !== undefined){
    ...
    config.emit("removedFromContext", whichOne, contextID);
  }
...
var addToLocalContext = ContextController.prototype.addToLocalContext = function (whichOne, contextID, callBack){
  ...
  if(typeof callBack === "function") callBack(false, contextID);
  config.emit("addedToContext", whichOne, contextID);
};
