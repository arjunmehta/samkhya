...
ConnectionInitializer.prototype.initialize = function(initializationMethods, done) {
    var moduleName;
    this.done = done;
    ...
    if (Object.keys(initializationMethods).length > 0) {
        ...
        for (moduleName in initializationMethods) {
            initializationMethods[moduleName].init(this.connection, buildInitializedClosure(this, moduleName));
        }
    } else {
        done();
    }
...
module.exports = ConnectionInitializer;
