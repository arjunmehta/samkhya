...
var createNewConnection = ConnectionController.prototype.createNewConnection = function(conn){
  ...
  samsaara.emit("connect", connections[currentID]);
...
ConnectionController.prototype.initializeConnection = function (connID, opts){
  ...
  if(opts !== undefined){
    ...
    if(opts.geoLocation !== undefined){
      ...
      communication.sendTo(connID, {internal: "getGeoLocation"}, geoPosition);
    }

    if(opts.windowSize !== undefined){
      if(opts.windowSize === "force") ia.windowSize = false;
      communication.sendTo(connID, {internal: "getWindowSize"}, windowResize);
    }
  }

  communication.sendTo(connID, {internal: "getNavInfo"}, getNavInfo); 
};
...
var doneInitializeConnection = ConnectionController.prototype.doneInitializeConnection = function (connID, whatInitialized){
  ...
  console.log(process.pid, moduleName, ia);

  if(ia[whatInitialized] !== undefined){
    ia[whatInitialized] = true;
  }

  console.log(process.pid, moduleName, whatInitialized, "INITIALIZED");
  ...
  if(allInitialized(ia)){
    if(whichOne.initialized === false){
      communication.sendTo(connID, {internal: "samsaaraInitialized", args: [connID]}, function(conn){
...
var testTime = ConnectionController.prototype.testTime = function (connID){
  ...
  if(connections[connID].navInfo.connectionTimings.afterMin < 10000000000){
    communication.sendTo(connID, {internal: "testTime", args:[( connections[connID].navInfo.connectionTimings.afterMin ), currentTime]}, testTimeReturn);
  }
  else{
    communication.sendTo(connID, {internal: "testTime", args:[0, currentTime]},  testTimeReturn);
  }
};
...
function testTimeReturn (originalTime, clientTime, timeError){
  ...
  if(this.navInfo.connectionTimings.latencies.length < timeAccuracy){
    ...
    communication.sendTo(this.id, {internal: "updateOffset", args: [this.navInfo.connectionTimings.clientOffset]});
...
var geoPosition = ConnectionController.prototype.geoPosition = function (err, geoposition){
  ...
  if(this.navInfo !== undefined){
    ...
    samsaara.emit('geoPosition', this, geoposition, err);
  }
  else{
    console.log(this);
  }
