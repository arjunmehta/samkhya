/*!
 * argyleSocks - Connection Initialization Methods
...
function heartBeater(){
  ...
  for(var connection in connections){
    ...
    console.log(process.pid, new Date().toTimeString(), "CONNECTION:", conn.id, conn.connectionClass);
...
var createNewConnection = ConnectionController.prototype.createNewConnection = function(conn){
  ...
  conn.write( JSON.stringify( {argyleID: currentID} ));
  conn.write( JSON.stringify( {argyleToken: connections[currentID].token} ));

  argyle.addToGroup(currentID, "everyone");

  //REDIS ADD
  if(config.options.redisStore){
    argyle.comStore.newConnection(currentID);
  }
...
function router(whichOne, message){
  ...
  // P (publish/broadcast) Used to send messages to a specified channel that connections can subscribe to (not yet part of argyle)
    ...
    switch(designation){
      ...
      default:
        if(whichOne.foreignContext !== null){
          ...
          argyle.redisPub.publish("CTX:" + whichOne.foreignContext, msgMod);
        }
        else{
          parseAndReceive();
        }
...
var removeConnection = ConnectionController.prototype.removeConnection = function(connID, message, callBack){
  ...
  var argyleGroups = argyle.groups;
  
  log.warn(" ", process.pid, moduleName, "CLOSING: ", connID);  

  //REDIS DELETE
  if(config.options.redisStore){
    argyle.comStore.closeConnection(connID);
  }

  authentication.removeConnectionSession(connID);

  if(connContext !== null && argyle.contexts[connContext] !== undefined){
    argyle.contexts[connContext].removeConnection(connID);
  }

  for(var key in argyleGroups){
    if(argyleGroups[key][connID] !== undefined){
      delete argyleGroups[key][connID];
    }
  }

  argyle.emit("disconnect", conn);
...
ConnectionController.prototype.initializeConnection = function (connID, opts){
  ...
  if(opts !== undefined){
    ...
    if(opts.groups !== undefined){
      ia.groups = false;
      argyle.addToGroup(whichOne.id, opts.groups, function(addedGroups){
...
var doneInitializeConnection = ConnectionController.prototype.doneInitializeConnection = function (connID, whatInitialized){
  ...
  if(allInitialized(ia)){
    if(whichOne.initialized === false){
      communication.sendTo(connID, {func: "argyleInitialized", args: [connID]}, function(conn){
        connections[conn].initialized = true;
        argyle.emit('initialized', connections[conn]);
      });
    }
  }
};
...
var windowResize = ConnectionController.prototype.windowResize = function (width, height, windowOffsetX, windowOffsetY){
  ...
  if(windowOffsetX){
    argyle.emit('windowSize', this, width, height, windowOffsetX, windowOffsetY);
  }
  else{
    argyle.emit('windowSize', this, width, height);
  }
};
...
var geoPosition = ConnectionController.prototype.geoPosition = function (geoposition){
  ...
  argyle.emit('geoPosition', this, geoposition);
};
...
var updateConnectionInfo = ConnectionController.prototype.updateConnectionInfo = function (attributes, callBack){
  ...
  if(whichOne.connectionClass === "symbolic"){
    argyle.comStore.sendToOwner(whichOne.id, whichOne.owner, {func: "updateConnectionInfo", args: [attributes], specialKey: argyle.specialKey}, callBack);
  }
};
