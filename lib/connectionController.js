    ...
    switch(designation){
      ...
      default:
        if(whichOne.foreignContext !== null){
          ...
          config.redisPub.publish("CTX:" + whichOne.foreignContext, msgMod);
        }
        else{
          parseAndReceive();
        }
...
var doneInitializeConnection = ConnectionController.prototype.doneInitializeConnection = function (connID, whatInitialized){
  ...
  // console.log(""+process.pid, moduleName, connID, ia);
  ...
  if(allInitialized(ia)){
    if(whichOne.initialized === false){
      communication.sendToClient(connID, {internal: "samsaaraInitialized", args: [connID]}, function(conn){
        connections[conn].initialized = true;
        config.emit('initialized', connections[conn]);
      });
    }
  }
...
var geoPosition = ConnectionController.prototype.geoPosition = function (err, geoposition){
  ...
  if(this.navInfo !== undefined){
    ...
    config.emit('geoPosition', this, geoposition, err);
  }
  else{
