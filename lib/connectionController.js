  ...
  (function connectionController(module){
    ...
    function checkForDeadConnections(){
      ...
      for(var connID in connections){
        ...
        // debug(samsaaraCore.uuid, new Date().toTimeString(), "CONNECTION Alive:", connection.id, connection.connectionClass);
        if(connection.owner === samsaaraCore.uuid && connection.lastHeartBeat < currentTimeOutTime){
