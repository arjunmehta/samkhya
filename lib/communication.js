    ...
    if(err === null){
      ...
      authentication.initiateUserToken( connectionController.connections[whichOne.id], loginObject.sessionID, loginObject.userID, function (err, token, userID){
        ...
        if(err !== null){
          ...
          communication.sendTo( whichOne.id, { internal: "reportError", args: [187, err, "Invalid Token Initiation: Session either Expired or Invalid"] });
        }
          ...
          communication.sendTo( whichOne.id, { internal: "updateToken", args: [whichOne.oldToken, token]}, function (token){
...
Communication.prototype.exposeNamespace = function(ns, as){
  if(samsaara.nameSpaces[as] === undefined){
...
function createCallBack(id, sender, owner){
  ...
  if(sender !== samsaara.specialKey){
    theCallBack = function(){
      ...
      communication.sendTo(sender, {internal: "callItBack", args: [id, owner, args] } );
    ...
    theCallBack = function(){
      ...
      samsaara.redisPub.publish(owner, JSON.stringify( {internal: "callItBack", args: [id, owner, args] }) );
      delete communication.outgoingCallBacks[id];
    };
  }
